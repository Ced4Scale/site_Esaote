...
// **Rechiffrée le 15/09/2026** (demande explicite de Cédric, remplace les valeurs
// individuelles précédentes par des repères génériques par catégorie) : Esaote (toutes
// machines sauf I-Genius) 12 min, IRM tunnel 1,5 T (aimant supraconducteur, toutes marques
// hors Esaote) 10 min, IRM tunnel 3 T 8 min. Non revues (catégories non couvertes par sa
// consigne) : Esaote I-Genius (18), IRM tunnel 7 T (Siemens Terra, 10), scanners CT
// (4, toutes marques).
//
// dureeChangementMinutes : temps de CHANGEMENT de patient — désinstallation du patient
// précédent + installation du suivant (déshabillage, positionnement, consignes), machine
// "ready to scan" une fois ce temps écoulé. EXCLUT l'acquisition elle-même (contrairement à
// l'ancien "dureeTotalePriseEnChargeMinutes", renommé le 14/09/2026 — voir plus bas) :
// le temps total avec un patient = dureeChangementMinutes + dureeAcquisitionMinutes,
// recalculé à chaque usage, jamais stocké tel quel. N'entre PAS dans le calcul de
// consommation électrique (déjà couvert par l'état "prêt"/"mesure"), mais sert (avec
// dureeAcquisitionMinutes) à vérifier qu'une activité saisie (patients/jour) reste
// physiquement réalisable.
// **Renommé et rechiffré le 14/09/2026** (demande explicite de Cédric) : l'ancien champ
// "dureeTotalePriseEnChargeMinutes" mesurait le temps TOTAL (installation+acquisition+
// sortie) ; il ne représentait plus correctement "Patients/h" une fois la table de mixage
// du simulateur énergie mise en place (une acquisition de 15 min limite déjà à 4
// patients/h à elle seule, indépendamment du changement).
// **Rechiffré une 2e fois le 15/09/2026** (nouvelle demande de Cédric) : Esaote O-scan
// 1,5 min (90 s, inchangé), Esaote G-scan/S-scan Open/Magnifico Open 2,5 min (relevé
// depuis 2 min), toute IRM à aimant supraconducteur (tunnel, toutes marques hors Esaote)
// 3 min (inchangé).
// **Non revues, en attente d'une vraie réponse de Cédric** (gardent l'ancien chiffre total
// tel quel, désormais interprété à tort comme un temps de changement seul — incohérence
// connue, pas une donnée validée) : Esaote I-Genius (15) et les scanners CT (10, toutes
// marques).
//
