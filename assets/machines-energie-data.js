...
//
// dureeTotalePriseEnChargeMinutes : durée TOTALE d'un créneau patient — installation
// (déshabillage, positionnement, consignes) + acquisition + déshabillage/sortie, l'ensemble
// du temps de prise en charge d'un examen. N'entre PAS dans le calcul de consommation
// électrique (déjà couvert par l'état "prêt"/"mesure" et par dureeAcquisitionMinutes,
// distinct), mais sert seul à vérifier qu'une activité saisie (patients/jour) reste
// physiquement réalisable — ne pas l'additionner à dureeAcquisitionMinutes, il l'inclut déjà.
// **Corrigé le 12/08/2026** : Cédric avait d'abord donné ces chiffres comme un temps
// d'installation séparé (avant l'acquisition), les modélisant additionnés à
// dureeAcquisitionMinutes — erreur de formulation de sa part, les chiffres visaient déjà le
// temps total. Chiffres confirmés le 12/08/2026 : 10 min pour un scanner, 15 min pour une IRM
// tunnel (aimant supraconducteur, toutes marques hors Esaote), et par modèle Esaote : O-scan
// 10, S-scan 15, G-scan 20 (corrigé depuis 15, seule valeur qui a changé), Magnifico 20,
// I-Genius 15.
//
...
var ENERGIE_MACHINES = [
  // ---------- Esaote — aimant permanent, pas de maintien en froid cryogénique ----------
  {
    ...
    dureeTotalePriseEnChargeMinutes: 10,
  ...
  },
  {
    ...
    dureeTotalePriseEnChargeMinutes: 15,
    coeffRefroidissementLT: 0,
    climSalleKw: 1.3,
    refroidissement: "aucun local technique dédié — climatisation de la salle d'examen seule, comme pour toute IRM",
  ...
  },
  {
    ...
    dureeTotalePriseEnChargeMinutes: 20,
    coeffRefroidissementLT: 0,
    climSalleKw: 1.5,
    refroidissement: "aucun local technique dédié — climatisation de la salle d'examen seule, comme pour toute IRM",
