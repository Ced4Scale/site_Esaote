...
- Rachis en charge : création de `examens-charge.html` avec lien "Voir examen en charge" depuis l'argument "Rachis en charge". Les séries G-Scan du dossier local fourni sont converties en JPEG anonymisés et limitées aux comparaisons 0° couché/81° en charge disponibles : 3D HYCE et T2 sagittal.
- Rachis en charge : ajout d'un deuxième cas G-Scan à partir de `Study-20105.26086-MR-5399[20241016]`, limité lui aussi aux paires 0°/81° disponibles : 3D HYCE coronal et FSE T2 REL sagittal. Les séries isolées 0° T1 et STIR ne sont pas publiées dans la page comparative.
- Examens métal / rachis : ajout d'une planche "vis par vis" reconstruite depuis le volume DICOM 3D HYCE. Six vis sont proposées sous forme de vignettes, chacune ouvrant par défaut un MPR 3 plans pré-calculé : axe de la vis, longitudinal complémentaire et orthogonal à la vis.
- Examens métal / rachis : remplacement de la planche "vis par vis" par une planche "MPR vis par vis" ; chaque vignette de vis est désormais elle-même un montage des 3 plans MPR synchronisés, tout en ouvrant le MPR interactif 3 plans au clic.
...
- Viewer examens métal / en charge sur iPhone : liste des séries rendue plus compacte en 2 colonnes avec tuiles réduites, intro raccourcie et vue d'ensemble plus rapide ; la comparaison conserve les images plus grandes.
